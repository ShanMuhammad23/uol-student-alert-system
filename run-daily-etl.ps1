...
function Invoke-EtlEndpoint {
  ...
  $url = "$($AppBaseUrl.TrimEnd('/'))$Endpoint"
  $attempt = 1
  ...
  while ($attempt -le $RetryCount) {
    ...
    try {
      ...
      $resp = Invoke-WebRequest -Method POST -Uri $url -Headers $headers -TimeoutSec $TimeoutSeconds
      Write-Host "SUCCESS $Endpoint -> Status: $($resp.StatusCode)"
      ...
      Write-Host "FAILED $Endpoint -> Status: $status"
...
foreach ($ep in $endpoints) {
  $results += Invoke-EtlEndpoint -Endpoint $ep
}
...
foreach ($r in $results) {
  ...
  Write-Host ("{0}  {1}" -f $state.PadRight(5), $r.Endpoint)
}
Write-Host "============================================="
