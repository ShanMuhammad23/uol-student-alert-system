...
FACULTY_ID="${INACTIVE_LOGIN_FACULTY_ID:-}"
INACTIVE_DAYS="${INACTIVE_LOGIN_INACTIVE_DAYS:-7}"
DRY_RUN="${INACTIVE_LOGIN_DRY_RUN:-false}"
...
mkdir -p "${LOG_DIR}"
LOG_FILE="${LOG_DIR}/inactive-login-$(date +%Y-%m-%d).log"
...
endpoint="/api/cron/inactive-login-reminders"
query=""
if [[ -n "${FACULTY_ID}" ]]; then
  query="facultyId=$(printf '%s' "${FACULTY_ID}" | sed 's/ /%20/g')"
fi
query="${query:+${query}&}inactiveDays=${INACTIVE_DAYS}"
...
if [[ -n "${FACULTY_ID}" ]]; then
  payload="$(printf '{"facultyId":"%s","inactiveDays":%s,"dryRun":%s}' \
    "${FACULTY_ID}" "${INACTIVE_DAYS}" "${dry_run_json}")"
else
  payload="$(printf '{"inactiveDays":%s,"dryRun":%s}' \
    "${INACTIVE_DAYS}" "${dry_run_json}")"
fi
...
log "Inactive login reminder run finished with errors"
exit 1
