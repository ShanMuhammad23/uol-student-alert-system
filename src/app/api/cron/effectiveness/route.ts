...
import { buildEffectivenessRows, upsertEffectivenessRows } from "@/lib/effectiveness";
  ...
  try {
    ...
    if (!facultyIds.length) {
      return NextResponse.json(
        { error: "facultyIds is required. Global effectiveness scores are disabled." },
        { status: 400 }
      );
    }

    const snapshotDate = new Date().toISOString().slice(0, 10);
    const rows = await buildEffectivenessRows(snapshotDate, { facultyIds });
    const upserted = await upsertEffectivenessRows(rows);
    ...
    console.error("Error in /api/cron/effectiveness:", error);
    return NextResponse.json(
      { error: "Failed to generate and save effectiveness scores", detail: message },
      { status: 500 }
    );
  }
}
